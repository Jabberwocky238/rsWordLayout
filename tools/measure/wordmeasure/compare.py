...
**但那个 0 是「重复性」，不是「准确度」。** 它量的是 Word 对 Word：同一份文档
两次导出，连舍入都一样，所以差为 0。拿它去读引擎对 Word 的 Δx 会读错一件事：
Word 导出的 PDF 把字形宽度写成 **1/1000 em 的整数**（22 份采集包、17231 个字形，
零例外），每个 run 再配**一个** `Tc` 把总量补回来。于是**行内**的字形位置带着
量化噪声，到**行末**才对齐。实测引擎行末 Δx max 2e-5pt、行中 max 0.0225pt，
而且 1340 个字形全部落在 `|Δx| ≤ 0.0005 × 字号 × 行内序号` 内——
那个 0.0225pt **不是引擎排错了，是这条通道就只有这个分辨率**。
见 `docs/MEASUREMENT-BACKLOG.md` 的 H6。

容差**仍然是 0**，这里不替它放水：要判横向的逐位相等，该在预注册里写明这条界，
而不是让比较器悄悄把一类差值吃掉。
"""
