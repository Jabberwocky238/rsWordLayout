#!/usr/bin/env python3
"""预注册判据的**可执行形式**（`docs/PREREG-2026-09-17-font-free.md`）。

与判据文同一次提交，**在采集之前**（量具方法 §7.1）。

判两条：W0 栅格、W1「`exact` 行距下基线位置与字体无关」。
W1 **不依赖任何公式**——只比同一 (`line`, 行序号) 上六个族的基线格数是否全等。
"""
...
def check_w0(lines):
    out = {"prediction": "W0 每条基线都在 0.24pt 栅格上", "n": 0, "ok": 0,
...
def check_w1(lines, probes, per_group):
    """W1：同一 (line, 行序号) 上，各族的基线格数必须全等。"""
    out = {"prediction": "W1 exact 行距下基线位置与字体无关", "n": 0, "ok": 0,
    ...
    for p in probes:
        ...
        if len(members) != per_group or len({m["page"] for m in members}) != 1:
            out["notes"].append({"tag": p["tag"], "family": p["family"],
                                 "lineTwips": p["lineTwips"],
                                 "reason": "该组未整组落在同一页，按采前声明排除"})
            seqs[(p["lineTwips"], p["family"])] = None
            continue
        seqs[(p["lineTwips"], p["family"])] = [
            int(round(m["baseline"] / GRID_PT)) for m in members]
        lo, hi = interval_for_A(seqs[(p["lineTwips"], p["family"])],
                               Fraction(p["lineTwips"] * 10, 48))
        out["intervals"].setdefault(p["lineTwips"], {})[p["family"]] = (
            f"[{float(lo % 1):.4f}, {float(hi % 1):.4f})" if lo < hi else "空")

    by_line = {}
    for (line, fam), seq in seqs.items():
        by_line.setdefault(line, {})[fam] = seq
    ...
    for line, fams in sorted(by_line.items()):
        for n in range(per_group):
            ...
            if len(set(vals.values())) == 1:
                ...
                out["misses"].append({"lineTwips": line, "n": n, "各族格数": vals})
    return out
...
def main():
    ...
    result = {"schema": "rsword-layout-prereg/1",
              "prereg": "docs/PREREG-2026-09-17-font-free.md",
    ...
    if fired:
        ...
        w0 = check_w0(lines)
        w1 = check_w1(lines, probes, per_group)
        result["predictions"] = {"W0": {**w0, "verdict": verdict(w0)},
                                 "W1": {**w1, "verdict": verdict(w1)}}
        print(f"采集包 state={result['bundleState']}  判定=EVALUATED")
        for key, p in (("W0", w0), ("W1", w1)):
        ...
        print("  A 的区间（另报，不作预测）：")
        for line, fams in sorted(w1["intervals"].items()):
            uniq = set(fams.values())
            print(f"      line={line:<5}{'各族一致 ' + next(iter(uniq)) if len(uniq)==1 else fams}")
