#!/usr/bin/env python3
"""预注册判据的**可执行形式**（`docs/PREREG-2026-09-17-hbox.md`）。

与判据文同一次提交，**在采集之前**（量具方法 §7.1）。

判四条：K0 栅格、K1 右对齐、K2 两端对齐、K3 字距。
...
def check_k0(lines):
    out = {"prediction": "K0 每条基线都在 0.24pt 栅格上", "n": 0, "ok": 0,
...
def check_k3(lines, probes):
    out = {"prediction": "K3 字距：Δ(sp,i) − Δ(0,i) = sp/2 pt", "n": 0, "ok": 0,
           "undecidable": 0, "misses": [], "notes": []}
    xs = {}
    for p in probes:
...
def main():
    ...
    result = {"schema": "rsword-layout-prereg/1",
              "prereg": "docs/PREREG-2026-09-17-hbox.md",
