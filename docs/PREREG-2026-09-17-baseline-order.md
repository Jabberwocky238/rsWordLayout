...
**上面那个「引擎形式」不是新形式，它就是 first-line 已经判假的 T1。**

那层「落到整 twips」**不起作用**，而且是结构性的不起作用：`quantize(x)` 是 4.8 twips
的倍数，落到整 twips 的残差 ≤ 0.5 twip = 0.025pt，而外层还有一次量化，半格是 0.12pt，
所以那点残差基本都被抹掉。在下一批夹具的 24 组上逐组核对，**含与不含这层取整，0 组不同**。

逐形式在三批上复核，得分完全一样：

| 批次 | 含 twips 取整 | 不含 | = T1 | hhea 形式 |
| --- | ---: | ---: | ---: | ---: |
| accumulation | 11/12 | 11/12 | 11/12 | 9/12 |
| first-line | 22/24 | 22/24 | 22/24 | 20/24 |
| baseline-order | 24/24 | 24/24 | 24/24 | 16/24 |

所以正确的说法是：**T1 仍是目前最好的形式（57/60），而它已经判假。**
「引擎实现了一个比所有候选都好的新形式」这句话是错的，就此更正。
